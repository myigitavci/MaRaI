...
from dist_clip.modules.resnet import ResNet50
...
class MedicalNetPerceptualSimilarity(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        # NOTE: we rely on MONAI's RadImageNet-based perceptual loss in this project,
        # so MedicalNet loading (which may depend on gdown) is not used at runtime.
...
class RadImageNetPerceptualSimilarity(nn.Module):
    ...
    def __init__(self, net: str = "radimagenet_resnet50", verbose: bool = False) -> None:
        ...
        try:
            ...
            self.model = ResNet50()
            state_dict = torch.load('/nfs/home/mavci/projects/AMA-CLIP/codes/MR-Styler/RadImageNet-ResNet50_notop.pth')
