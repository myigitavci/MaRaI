...
def get_latest_checkpoint(path: str, remote : bool):
    ...
    return None
def update_dropout(model, p=0.4):
    """Recursively updates all dropout layers in the model to have a probability `p`."""
    for name, module in model.named_modules():
        if isinstance(module, nn.Dropout):
            module.p = p  # Update the probability in-place
            module.inplace = False  # Ensure inplace=False to avoid issues
...
def main(args):
    ...
    model, preprocess_train, preprocess_val = create_model_and_transforms('hf-hub:microsoft/BiomedCLIP-PubMedBERT_256-vit_base_patch16_224',
        args.pretrained,
    ...
    # Apply to your HFTextEncoder model
    if args.textdropout is not None:
        update_dropout(model.text.transformer,args.textdropout)
    ...
    if args.distributed and not args.horovod:
        ...
        if args.distill:
            dist_model = torch.nn.parallel.DistributedDataParallel(dist_model, find_unused_parameters=True, device_ids=[device], **ddp_args)
